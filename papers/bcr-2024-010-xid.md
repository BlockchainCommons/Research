...
The set of top-level assertions in a XID document is intentionally open. This specification defines the semantics of assertions such as `key`, `delegate`, `dereferenceVia`, `service`, `provenance`, `attachment`, and `edge`, but a XID document MAY include additional assertions whose predicates are not defined by this specification. A processor that does not understand such an assertion MAY ignore it when evaluating XID semantics, but when producing an updated XID document it MUST preserve all unrecognized top-level assertions unchanged unless explicitly directed to remove them. This preservation requirement does not relax validation of assertions whose predicates are defined by this specification; recognized assertions that are malformed MUST be rejected according to their definitions.

This rule applies to top-level assertions about the XID. Nested structures MAY define stricter schemas.
