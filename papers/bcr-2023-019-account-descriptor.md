# UR Type Definition for BIP44 Accounts (Version 2)
...
This information can be contained in an output descriptor [OD-IN-CORE], which is defined in CBOR with [BCR-2023-010] and represented in UR using the `ur:output-descriptor` type.
Using this format, each output descriptor defines a single script type, derivation path and parent fingerprint (`keypath`) and deterministic key (`hdkey`).
...
This encoding can be shared as a Uniform Resource [UR] type `account-descriptor` (CBOR tag #6.40311) for transmitting account level data.

In the encoding, each script type is represented in an output descriptor, `output-descriptor`, which describes the script type and its associated public key information.

In the case of multisig script types, a partial output descriptor is used, which contains the public key information for a single cosigner. This is expressed as an extension to the output descriptor specification [OD-IN-CORE] by adding a new descriptor function `cosigner(KEY)`, which can be accepted by the `sh` and `wsh` script functions defined in [OD-IN-CORE]. The `cosigner(KEY)` is not used to derive ScriptPubKeys directly, but is used as a placeholder to be used when transmitting a single public key representing an account to be used by a party in a multisig transaction. Please see the [output-descriptor](bcr-2023-010-output-descriptor.md#cosigner) specification for more details.

The top level packaging of `account-descriptor` is a map of the master fingerprint, and the array of output descriptors `output-descriptor`.

Since each `hdkey` embedded within a `output-descriptor` may or may not include the master fingerprint, `account-descriptor` includes the master fingerprint at its top level. The following resolution rule applies: If the `hdkey` contains the master fingerprint, then that fingerprint is the source of truth for that key, but if it omits the master fingerprint, then the master fingerprint at the top level of the `account-descriptor` structure is the source of truth.

The following standardized script types may be present in a `account-descriptor` encoding, shown here with default derivations for Bitcoin mainnet and account #0:
...
The previous version of this specification [[BCR-2020-015]](bcr-2020-015-account.md) suffered from the same problem described in [[BCR-2023-010]](bcr-2023-010-output-descriptor.md#previous-versions). While the output descriptors in this format are likely to be simple, the format needed to be updated for the Version 3 output descriptor schema [[BCR-2023-010]](bcr-2023-010-output-descriptor.md).
...
Note that the above output descriptors take 996 bytes to encode as text, while the encoded CBOR for the `account-descriptor` that encodes them takes only 859 bytes-- an over 8% savings. Further gains are realized when using the UR format to transmit, including human-friendly text encoding as URIs, error detection, efficiency of QR code encoding, and the ability to break up messages too large to fit into a QR code into a sequence of QR codes using sophisticated "fountain codes."

* In CBOR diagnostic form:

(Note the `hdkey` `use-info` field is omitted here because it takes on the default value for `asset` and `network`, include for other networks.)
   ...
   87                                   # array(7)
      ...
      D9 9D74                           # tag(40308)
         A2                             # map(2)
            01                          # unsigned(1)
            70                          # text(16)
               736828636F7369676E65722840302929 # "sh(cosigner(@0))"
            02                          # unsigned(2)
            81                          # array(1)
               D9 9D6F                  # tag(40303)
                  A4                    # map(4)
      ...
      D9 9D74                           # tag(40308)
         A2                             # map(2)
            01                          # unsigned(1)
            75                          # text(21)
               73682877736828636F7369676E6572284030292929 # "sh(wsh(cosigner(@0)))"
            02                          # unsigned(2)
            81                          # array(1)
               D9 9D6F                  # tag(40303)
                  A4                    # map(4)
...
```

```

* As a UR:

```
ur:account-
```
...
* [BCR-2023-010] [UR Type Definition for Bitcoin Output Descriptors (Version 3)](https://github.com/BlockchainCommons/Research/blob/master/papers/bcr-2023-010-output-descriptor.md)
