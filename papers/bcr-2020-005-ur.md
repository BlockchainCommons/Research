...
The only type this document specifies is `bytes` which represents an undifferentiated string of bytes of any length. The `bytes` exist for testing and validation of UR implementations and MUST NOT be used for any other purpose. It is intended that future specifications will register and document other types that will specify other forms of structured content intended to address various problem domains.
