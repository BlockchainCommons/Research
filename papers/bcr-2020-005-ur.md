...
* Support transmitting an arbitrary amount of data both as a minimal, finite sequence of parts *and* as an indefinite sequence of parts using rateless encoding a [Fountain Code](https://en.wikipedia.org/wiki/Fountain_code) based on [Luby Transform code](https://en.wikipedia.org/wiki/Luby_transform_code).
...
part
: Refers to a multi-part UR as one of a set of URs that together convey the message.
...
The method of encoding binary data as printable characters specified in this proposal is [Bytewords](https://github.com/BlockchainCommons/Research/blob/master/papers/bcr-0012-bytewords.md).
...
The reader is referred to [URKit](https://github.com/BlockchainCommons/URKit), which is the Swift reference implementation of all the structures and algorithms described herein, including a [set of unit tests](https://github.com/BlockchainCommons/URKit/tree/master/Tests/URKitTests) that other compliant implementations MUST pass. For an interactive demonstration of single and multi-part encoding and decoding using URKit, the reader is referred to the iOS application [URDemo](https://github.com/blockchaincommons/URDemo).
...
Both the degree chooser and fragment shuffle algorithm take the Xoshiro RNG as an input, resulting in a deterministic outcome that the encoder and decoder agree upon.
